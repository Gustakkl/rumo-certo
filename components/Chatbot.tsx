...
const Chatbot: React.FC = () => {
  ...
  return (
    <>
      <div className={`fixed bottom-24 right-5 w-80 h-[28rem] bg-teal-900 rounded-lg shadow-2xl flex flex-col transition-all duration-300 ease-in-out z-50 ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
        <div className="bg-teal-950 p-3 flex items-center space-x-3 rounded-t-lg shadow-md">
          ...
          <div>
            <h3 className="font-bold text-white">Gabi</h3>
        ...
        <div ref={chatboxRef} className="flex-1 p-4 overflow-y-auto space-y-4">
          {messages.map((msg, index) => (
            <div key={index} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] px-3 py-2 rounded-lg shadow whitespace-pre-wrap ${msg.sender === 'user' ? 'bg-amber-500 text-white rounded-br-none' : 'bg-teal-800 text-gray-200 rounded-bl-none'}`}>
                {msg.text}
              </div>
            </div>
          ))}
          {showSuggestions && (
            <div className="flex flex-col items-start space-y-2 pt-2">
                {initialSuggestions.map(sugg => (
                    <button key={sugg} onClick={() => processMessage(sugg)} className="text-sm bg-teal-800 text-amber-300 px-3 py-1 rounded-full hover:bg-teal-700 transition-colors disabled:opacity-50" disabled={isLoading}>
                        {sugg}
                    </button>
                ))}
            </div>
          ...
          {isLoading && messages[messages.length - 1]?.sender === 'user' && (
             <div className="flex justify-start">
                <div className="bg-teal-800 text-gray-200 rounded-bl-none px-3 py-2 rounded-lg flex items-center space-x-1 shadow">
        ...
        <form onSubmit={handleSendMessage} className="p-3 border-t border-teal-800">
          <div className="flex space-x-2">
            <input 
              ...
              className="w-full bg-teal-800 text-white rounded-md p-2 border border-teal-700 focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all text-sm"
              disabled={isLoading}
            />
